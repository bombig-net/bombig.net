...
const BadgeFrosted: React.FC<BadgeFrostedProps> = ({ children, className }) => {
    return (
        <div
            className={cn("relative border-jordy-500/20 bg-jordy-300/10 opacity-80 backdrop-blur-sm mt-4 px-2 lg:px-4 py-1.5 lg:py-2 border rounded-full text-center text-jordy-200/70 text-xs lg:text-sm uppercase tracking-wider", className)}
        >
            {children}
